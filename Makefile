...
kind-node-image: ## Build the kindest/node image with nfs-common baked in.
	docker build -t $(KIND_NODE_IMG) -f hack/kind/node.Dockerfile hack/kind
...
.PHONY: kind-prep
kind-prep: ## Install nfs-common into an existing (stock-image) kind cluster's nodes.
	@for n in $$($(KIND) get nodes --name $(KIND_CLUSTER)); do \
		echo "installing nfs-common on $$n"; \
		docker exec $$n sh -c 'command -v mount.nfs >/dev/null || (apt-get update && apt-get install -y --no-install-recommends nfs-common)'; \
	done
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name nfsz-builder
	$(CONTAINER_TOOL) buildx use nfsz-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm nfsz-builder
	rm Dockerfile.cross
...
undeploy: kustomize ## Undeploy controller from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	@# Drain SharedVolumes while the controller can still run their
	@# finalizers; deleting the controller first leaves CRs that can never
	@# finalize, and the subsequent CRD deletion hangs forever on them.
	-"$(KUBECTL)" delete sharedvolumes --all --timeout=120s 2>/dev/null || true
